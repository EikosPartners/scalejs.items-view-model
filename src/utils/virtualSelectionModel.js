...
export default function (options) {
    ...
    var viewModel = options.viewModel, //Should be the itemsViewModel reference
    ...
    function init(grid) {
        ...
        var appendContextMenu = function(body) {
            $contextMenu = $(document.querySelector(options.contextMenuSelector));
        ...
        viewModelItemsSub = viewModel.items.subscribe(wrapTimeoutHandler(function () { selectRows(); }));
    ...
    function wireColumnDragDrop(args) {
        ...
        var $wndBody = $(document).find("body");
        var gridColumns;
        ...
        function columnMouseDown(e) {
            ...
            mouseDownTimeout = setTimeout(function () {
                ...
                $draggingColumnDiv = $target
                    .clone()
                    .attr("id", "columnDragCopy")
                    .addClass("slick-header-column-dragging")
                    .appendTo($wndBody);
        ...
        function endDraggingCleanup(e) {
            ...
            //Reset divs
            if ($draggingColumnDiv != null)
                $draggingColumnDiv.remove();

            if ($sourceColumnDiv != null)
                $sourceColumnDiv.css("opacity", sourceColumnOrigOpacity);
        }
        ...
        function columnMouseEnter(e) {
            ...
            var reorderedIds = cols
                .select(function(col, idx) { return { id: col.id, idx: idx }; })
                .toArray();

            //Move the original column
            var oldidx = reorderedIds
                .first(function(rid) { return rid.id === lastColumnId; })
                .idx;

            var newidx = reorderedIds
                .first(function(rid) { return rid.id === dropColumn.id; })
                .idx;
            ...
            for (var i = 0; i < reorderedIds.length; i++) {
                var origCol = cols.first(function(c) { return c.id === reorderedIds[i].id; });
    ...
    function wireColumnArrangment() {
        ...
        function columnMouseOver(e) {
            ...
            //Store the current column id
            $sourceColumnDiv = $(e.target);
            if ($sourceColumnDiv.is("span"))
                $sourceColumnDiv = $sourceColumnDiv.parent();
            ...
            resizeTimeout = setTimeout(function() {
                var offset = $sourceColumnDiv.offset();
                 ...
                 $resizeDivRight = $('<div id="divResizeRight"></div>')
                    .height($sourceColumnDiv.outerHeight())
                    .addClass('slick-resizable-handle')
                    .offset({ left: offset.left + $sourceColumnDiv.outerWidth(), top: offset.top })
                    .appendTo($wndBody);
                ...
                //Determine the previous column
                 ...
                 var curridx = cols
                     .select(function (c, idx) { return { id: c.id, idx: idx }; })
                     .first(function (c) { return c.id === sourceColumnId; })
                     .idx;

                 priorColumnId = curridx > 0 ? cols[curridx - 1].id : null;

                 if (priorColumnId != null) {
                     $priorColumnDiv = $(gridColumns.first(function(c) { return $(c).data('column').id === priorColumnId; }));

                     $resizeDivLeft = $('<div id="divResizeLeft"></div>')
                         .height($sourceColumnDiv.outerHeight())
                         .addClass('slick-resizable-handle')
        ...
        function resizeMove(e) {
            ...
            if ($resizeDivRight != null) {
                ...
                localCol = columns().first(function (c) { return c.id === sourceColumnId; });
            } else {
                ...
                localCol = columns().first(function (c) { return c.id === priorColumnId; });
            }
        ...
        function endResizingCleanup(e) {
            ...
            if (e != null &&
                (($resizeDivRight != null && e.toElement === $resizeDivRight[0])
                 || ($resizeDivLeft != null && e.toElement === $resizeDivLeft[0])))
                return;
            ...
            if (parentRow != null)
                parentRow.removeEventListener('mouseleave', endResizingCleanup);

            if ($wndBody != null)
                $wndBody[0].removeEventListener('mouseleave', endResizingCleanup);
        }

        //Simulate mouse over exposed for the benefit of drag-drop
        function triggerColumnMouseOver(columnId) {
            var col = gridColumns.first(function (c) { return $(c).data('column').id === columnId; });
            var e = { target: col };
            columnMouseOver(e);
        }
    ...
    function clearSelection() {
        if (getSelectedRows().length > 0) {
            options.contextMenuSelector && $(options.contextMenuSelector).hide();
            onSelectEnd.next();
        }
    ...
    function getItemKey(item) {
        ...
        return keyFields.map(function (field) { return item[field]; }).join("|");
    }
    ...
    function commitCellEdit(continueEditStatus) {
        ...
        if (_grid == null || _grid.getCellEditor() == null)
            return;
        ...
        var e = $.extend(true, {}, _grid.getActiveCell());
        e.grid = _grid;
        e.item = rowObject;
    ...
    function handleKeyDown(evt, data) {
        ...
        if (!evt.altKey && !evt.metaKey && (evt.which === 37 || evt.which === 38 || evt.which === 39 || evt.which === 40 || evt.which === 9)) {
            ...
            data.cell = columns().length - 1;
            if (evt.ctrlKey)
                data.row = _grid.getDataLength() - 1;
    ...
    function handleClick(evt, data) {
        ...
        if (evt.isFromKeyEvent === true)
            var cell = { cell: data.cell, row: data.row };
        else
            cell = _grid.getCellFromEvent(evt);
        ...
        if (selectedItem !== undefined) {
            ...
            if (evt.ctrlKey && !evt.isFromKeyEvent) {
                // Ctrl click
                if (selected[selectedItemKey] === undefined)
                    selected[selectedItemKey] = selectedItem;
                else if (!evt.isFromContextMenuEvent)
                    delete selected[selectedItemKey];

            } else if ((Object.keys(selected).length === 0 || ((evt.isFromKeyEvent || !evt.shiftKey) && !evt.altKey && !evt.metaKey))
                        && !(!evt.ctrlKey && evt.isFromContextMenuEvent && selected[selectedItemKey] != null)) {
        ...
        if (!columns()[cell.cell].hasOwnProperty("editor")) {
            ...
            selectRows(function() { _grid.editActiveCell(); });
        };
    ...
    function handleContextMenu(evt) {
        ...
        if (contextWindow && !(evt.shiftKey || evt.altKey || evt.metaKey)) {
            ...
            if (options.onContextMenu)
                options.onContextMenu.call(_grid, evt);
        }
    ...
    function flashSelectedRow() {
        flashTimeouts.forEach(function (t) { clearTimeout(t); });
        ...
        var interval = 100;
        for (var i = 0; i < 7; i++) {
            flashTimeouts.push(setTimeout(function () { setSelectedRows([]); }, i * interval));
            flashTimeouts.push(setTimeout(function () { setSelectedRows(rows); }, (i + 1) * interval));
        }

        flashTimeouts.push(setTimeout(function () { viewModel.unpauseConnectionStream(); }, (i + 5) * interval));
