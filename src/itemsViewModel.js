...
export default function (containerId, columns, keyFields, createServerConnection, selectionOptions, defaultSorting) {
    ...
    var cols = columns().filter(function (column) { return column.filter; }),
        lastFiltersJson,
            ...
            for (var i = 0; i < cols.length; i += 1) {
                ...
                if (filterValue != null) {
                    for (var j = 0; j < filterValue.length; j += 1) {
                        ...
                        if (filterValue[j].tag != null)
                            f.tag = filterValue[j].tag;

                        if (filterValue[j].logicOperator != null)
                            f.logicOperator = filterValue[j].logicOperator;
    ...
    function subscribeToWindowItems() {
        return serverConnection.windowBus
    ...
    function subscribeToQuickSearchInput() {
        return Observable
            .fromArray(columns().filter(function (column) { return column.filter && column.filter.quickSearch; }))
            .selectMany(function (column) {
                return Observable
                    .fromKoObservable(column.filter.quickSearch)
                    .where(function(quickSearchValue) {
            ...
            .subscribe(function (quickSearchExpression) {
                ...
                columns()
                    .first(function (column) { return column.field === quickSearchExpression.column })
    ...
    viewModel.selectionModel = new selectionModel(merge({
        viewModel: viewModel
    }, selectionOptions));
