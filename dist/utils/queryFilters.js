...
exports.default = toQueryFilters;
...
function toQueryFilters(filters) {
    ...
    filters.forEach(function (filter) {
        ...
        if (queryOperator.name !== "NotEmpty") {
            var values = queryOperator.cast != null ? filter.values.map(function (val) {
                return queryOperator.cast(val);
            }) : filter.values;
        ...
        if (filter.tag != null) queryFilters.last().tag = filter.tag;

        if (filter.logicOperator != null) queryFilters.last().logicOperator = filter.logicOperator;
    });
    return queryFilters;
}