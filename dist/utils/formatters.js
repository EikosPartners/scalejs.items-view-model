...
exports.algoFormatter = exports.convertQualifiedNumber = exports.accountFormatter = exports.buySellMiniFormatter = exports.quantityFormatter = exports.testFormatter = exports.dropDownFormatter = exports.mdQualityFormatter = exports.progressBarFormatter = exports.buySellFormatter = exports.moneyFormatterMM = exports.moneyFormatterK = exports.moneyFormatter = exports.isYesNoFormatter = exports.isBoolFormatter = exports.timeUTCFormatter = exports.timeLocalFormatter = exports.timeFormatter = exports.priceFormatter = exports.defaultFormatter = undefined;
...
function formatNumber(number, minDigits, maxDigits, allowQualifiers) {
    ...
    if (allowQualifiers) {
        ...
        if (proposed.indexOf('k', proposed.length - 1) !== -1) number = Number(proposed.substring(0, proposed.length - 1)) * 1000;else if (proposed.indexOf('m', proposed.length - 1) !== -1) number = Number(proposed.substring(0, proposed.length - 1)) * 1000000;else number = Number(proposed);
    }
    ...
    if (integer === 0) {
        ...
        do {
            ...
            if (integer != 0) {
                ...
                if (part === 0) part = '000';else if (part < 10) part = '00' + part;else if (part < 100) part = '0' + part;
                str = "," + part + str;
            } else {
                str = part + str;
            }
...
function priceFormatter(row, cell, value, columnDef, dataContext) {
    return (0, _coreFunctions.has)(value) ? formatNumber(+value, 2, 5, true) : ''; //(+value).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 5 });
}

function timeFormatter(row, cell, value, columnDef, dataContext) {
    return (0, _coreFunctions.has)(value) ? date.toString(value, 'yyyy-MM-dd HH:mm:ss.fff') : '';
}
...
function timeLocalFormatter(row, cell, value, columnDef, dataContext) {
    ...
    return (0, _coreFunctions.has)(value) && value != "" ? date.toString(value, 'yyyy-MM-dd HH:mm:ss.fff') : '';
}

function timeUTCFormatter(row, cell, value, columnDef, dataContext) {
    return (0, _coreFunctions.has)(value) && value != "" ? value.substr(0, 10) + ' ' + value.substr(11, 12) : ''; // Faster
...
function isBoolFormatter(row, cell, value, columnDef, dataContext) {
    return (0, _coreFunctions.has)(value) ? value ? 'YES' : 'NO' : 'NO';
}

function isYesNoFormatter(row, cell, value, columnDef, dataContext) {
    return (0, _coreFunctions.has)(value) ? value.toLowerCase() === "yes" ? 'YES' : 'NO' : 'NO';
}
...
function progressBarFormatter(row, cell, value, columnDef, dataContext) {
    if (value) {
        ...
        value = (0, _coreFunctions.has)(value) ? formatNumber(value, 0, 0) : '';
        ...
        return (0, _coreFunctions.has)(value) ? formatNumber(value, 2, 2) : '';
...
function algoFormatter(row, cell, value, columnDef, dataContext) {
    var algo = '';
    if (value == null || value === '') return algo;
...
function mdQualityFormatter(row, cell, value, columnDef, dataContext) {
    ...
    if (dataContext.mdQuality != "REALTIME") cssCell += "slick-cell-delayed";
    ...
    if (dataContext.hasOwnProperty("staleWarning") && (dataContext.staleWarning === true || dataContext.staleWarning === "true")) {
        if (cssCell.length === 0) cssCell = "slick-cell-stale";else cssCell += " stale";
    }

    //If there is a class a div is needed
    if (cssCell.length > 0) return '<div class="' + cssCell + '">' + value + '</div>';else return value;
}
...
function quantityFormatter(row, cell, value, columnDef, dataContext) {
    ...
    return (0, _coreFunctions.has)(value) ? formatNumber(value, 0, 0, true) : '';
    //return has(value) ? text.numberFormat(value, 0, '.', ',') : '';
}
...
function convertQualifiedNumber(numString) {
    ...
    if (numString == null) return numString;
    ...
    if (idx > -1) {
        ...
        //Convert to number
        if (numdec > 0) proposed = proposed + new Array(numdec + 1).join('0');else if (numdec < 0) proposed = proposed.substring(0, proposed.length - 2 - numdec) + '.' + proposed.substring(proposed.length - 2 - numdec);
    };
...
exports.timeLocalFormatter = timeLocalFormatter;
exports.timeUTCFormatter = timeUTCFormatter;
exports.isBoolFormatter = isBoolFormatter;
exports.isYesNoFormatter = isYesNoFormatter;
exports.moneyFormatter = moneyFormatter;
exports.moneyFormatterK = moneyFormatterK;
exports.moneyFormatterMM = moneyFormatterMM;
exports.buySellFormatter = buySellFormatter;
exports.progressBarFormatter = progressBarFormatter;
exports.mdQualityFormatter = mdQualityFormatter;
exports.dropDownFormatter = dropDownFormatter;
exports.testFormatter = testFormatter;
exports.quantityFormatter = quantityFormatter;
exports.buySellMiniFormatter = buySellMiniFormatter;
exports.accountFormatter = accountFormatter;
exports.convertQualifiedNumber = convertQualifiedNumber;
exports.algoFormatter = algoFormatter;